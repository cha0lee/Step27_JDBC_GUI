...
public class MemberFrame extends JFrame 
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(command.equals("save")) {
			...
			if(isSuccess) {
				JOptionPane
				.showMessageDialog(this, "저장했습니다.");
			}else {
				JOptionPane
				.showMessageDialog(this, "저장실패!");
			}
		}else if(command.equals("delete")) {//삭제 버튼을 눌렀을때
			//예, 아니요, 취소 중에 어떤 버튼을 눌렀는지 정보가
			//int type 으로 리턴된다. 
			int result=JOptionPane.showConfirmDialog(this, "진짜 삭제할겨?");
			// 예 버튼을 누르지 않았다면 
			...
			if(selectedIndex==-1) {
				JOptionPane
				.showMessageDialog(this,"삭제할 row 를 선택하세요.");
				return;//메소드 종료 
			}
			...
			if(selectedIndex==-1) {
				JOptionPane
				.showMessageDialog(this,"수정할 row 를 선택하세요.");
				return;//메소드 종료 
			}
			//수정할 회원정보를 읽어와서 
			int num=(int)table.getValueAt(selectedIndex, 0);
			...
			JOptionPane
			.showMessageDialog(this,"수정하였습니다.");
		}
		//회원 정보 다시 출력
		displayMember();
	}//actionPerformed()
